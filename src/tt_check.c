...
lizard_ast_node_t *ctx_lookup(lizard_ast_node_t *ctx,
                                     const char *name) {
...
lizard_ast_node_t *ctx_extend(lizard_ast_node_t *ctx,
                                     const char *name,
...
lizard_ast_node_t *type_error(lizard_heap_t *heap,
                                     const char *msg) {
  (void)msg;
...
lizard_ast_node_t *make_path_app_local(lizard_heap_t *heap,
                                              lizard_ast_node_t *p,
                                              lizard_ast_node_t *i) {
...
static lizard_ast_node_t *infer2_kind_impl(
  ...
  case AST_TT_CO_SIGMA_FRESH:
    return infer2_fresh(valid_ctx, ctx, t, heap, out_kind);
  case AST_TT_BOX:
  case AST_TT_DIAMOND:
  ...
  case AST_TT_DIAMOND_BIND:
    return infer2_modal(valid_ctx, ctx, t, heap, out_kind);
  case AST_TT_APP: {
  ...
  case AST_TT_TRUNC_ELIM:
    return infer2_hit(valid_ctx, ctx, t, heap, out_kind);
  case AST_TT_EXTENSION:
    if (lizard_logic_rule_enabled("theory-extensions-enabled") != 1) {
      return type_error(heap, "theory-extension rejected: theory-extensions-enabled disabled");
    }
    return type_error(heap, "theory-extension has no kernel typing rule");
  case AST_TT_UNIT:
  ...
  case AST_TT_UNGLUE:
  case AST_TT_UA:
    return infer2_cubical(valid_ctx, ctx, t, heap, out_kind);
  case AST_TT_LAMBDA:
  ...
  case AST_TT_POSS_COERCE: {
     ...
     *
     * Type unchanged; kind shifted. Gated on `modal-symmetric`. */
