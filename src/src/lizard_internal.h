#ifndef LIZARD_INTERNAL_H
#define LIZARD_INTERNAL_H
...
typedef enum {
  ...
  AST_TT_GLUE_SYS,
  /* Optional proof-theory scaffolds. These are opt-in via logic rules
   * and are not part of the trusted kernel yet. */
  AST_TT_S1,
  AST_TT_S1_BASE,
  AST_TT_S1_LOOP,
  AST_TT_TRUNC,
...
struct lizard_ast_node {
  ...
  union {
    ...
    struct {
      ...
      lizard_ast_node_t *handler;
      lizard_ast_node_t *prop;      /* optional: Π x:C. Π y:C. Path C x y */
      lizard_ast_node_t *value;
    } tt_trunc_elim;
    struct {
      lizard_ast_node_t *name;
