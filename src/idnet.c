...
static void rule(int a, int d) {
  ...
  if (ka == N_ID) {
    ...
    if (kd == TY_PROD) {
      /* Id_(A*B)((a,b),(c,d)) = (Id A a c) * (Id B b d) */
      int P  = mk(TY_PROD);
    ...
    /* unsupported former (functions / dependent): leave inert -- read-back refuses */
    return;
  }
  if (ka == N_IDB) {
  ...
  }
  {
    ...
    switch (k_[a]) {
      ...
      case VAL_PAIR: r = id_pair(rb(pt_[a*PORTS+1], pp_[a*PORTS+1]),
                                 rb(pt_[a*PORTS+2], pp_[a*PORTS+2])); break;
      default:       rb_bad = 1; r = id_base(ID_UNIT); break;   /* ID/UNPAIR/ERA left in a term slot */
    }
