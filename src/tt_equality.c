...
/* Universe/couniverse lattice helper prototypes live in tt_lattice.h. */
...
/* Equivalence, Glue, ua, and system constructors live in tt_glue.c. */
...
/* Universe/couniverse lattice constructors and set helpers live in tt_lattice.c. */
...
static lizard_ast_node_t *try_head_rewrites(lizard_ast_node_t *t,
  ...
  case AST_TT_U_MAX: {
    ...
    if (l->type == AST_TT_UNIVERSE_SET && r->type == AST_TT_UNIVERSE_SET &&
      ...
      *changed = 1;
      return lizard_tt_universe_set_union(heap, l, r);
    }
    ...
    if (l->type == AST_TT_UNIVERSE && r->type == AST_TT_UNIVERSE_SET &&
      ...
      *changed = 1;
      return lizard_tt_universe_set_union(heap, lift, r);
    }
    if (l->type == AST_TT_UNIVERSE_SET && r->type == AST_TT_UNIVERSE &&
      ...
      lift = lizard_tt_make_universe_set(heap, dim, 1);
      *changed = 1;
      return lizard_tt_universe_set_union(heap, l, lift);
    }
  ...
  case AST_TT_U_MIN: {
    ...
    if (l->type == AST_TT_UNIVERSE_SET && r->type == AST_TT_UNIVERSE_SET &&
        lizard_tt_flag_get("reduce-u-set-concrete")) {
      *changed = 1;
      return lizard_tt_universe_set_intersect(heap, l, r);
    }
    if (l->type == AST_TT_UNIVERSE && r->type == AST_TT_UNIVERSE_SET &&
        lizard_tt_flag_get("reduce-u-set-concrete")) {
      long *dim = lizard_heap_alloc(sizeof(long));
      ...
      lift = lizard_tt_make_universe_set(heap, dim, 1);
      *changed = 1;
      return lizard_tt_universe_set_intersect(heap, lift, r);
    }
    if (l->type == AST_TT_UNIVERSE_SET && r->type == AST_TT_UNIVERSE &&
        lizard_tt_flag_get("reduce-u-set-concrete")) {
      long *dim = lizard_heap_alloc(sizeof(long));
      ...
      lift = lizard_tt_make_universe_set(heap, dim, 1);
      *changed = 1;
      return lizard_tt_universe_set_intersect(heap, l, lift);
    }
    if (l->type == AST_TT_UNIVERSE && r->type == AST_TT_UNIVERSE &&
  ...
  case AST_TT_CO_MAX: {
    ...
    if (l->type == AST_TT_COUNIVERSE_SET && r->type == AST_TT_COUNIVERSE_SET &&
      ...
      *changed = 1;
      return lizard_tt_couniverse_set_union(heap, l, r);
    }
    if (lizard_tt_alpha_equal(l, r) &&
  ...
  case AST_TT_CO_MIN: {
    ...
    if (l->type == AST_TT_COUNIVERSE_SET && r->type == AST_TT_COUNIVERSE_SET &&
        lizard_tt_flag_get("reduce-co-set-concrete")) {
      *changed = 1;
      return lizard_tt_couniverse_set_intersect(heap, l, r);
    }
    if (lizard_tt_alpha_equal(l, r) &&
...
/* Fresh dimensions, HIT registry, and logic-rule configuration live in tt_registry.c. */
...
/* Universe/couniverse ordering lives in tt_lattice.c. */
...
/* Face entailment and system lookup live in tt_faces.c. */
