...
lizard_ast_node_t *lizard_get_canonical_nil(lizard_heap_t *heap) {
  ...
  if (!canonical_empty_list) {
    ...
    canonical_empty_list->type = AST_NIL;
  }
      ...
      if (strcmp(current_token->data.symbol, "quote") == 0) {
        ...
        *current_node_pointer = current_node->next;
        ...
        if (var_node->type == AST_APPLICATION) {
          ...
          /* Read one-or-more body expressions until we hit the closing
             paren of the define. The lambda's parameter list, by lizard's
             convention, has the param-application at position 0 and the
             body expressions at positions 1..N. */
          {
            int saw_body = 0;
            while (1) {
              current_node = *current_node_pointer;
        ...
        while (1) {
          ...
          *current_node_pointer = current_node->next;
          ...
          var = lizard_parse_expression(token_list, current_node_pointer, depth,
                                        heap);
          current_node = *current_node_pointer;
          /* Normally var must be a symbol, but if we're parsing inside a
             quasiquote literal (where var may be an AST_UNQUOTE that will
             be substituted with a real symbol at expansion time) we let it
             pass and rely on eval to surface the error if expansion does
