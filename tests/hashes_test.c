...
int main(void) {
  ...
  /* Basic set + ref. */
  r = lizard_test_eval(&e, "(define h (make-hash-table))"
  ...
  r = lizard_test_eval(&e, "(hash-set! h 'age 6)"
                           "(hash-size h)");
  TEST_ASSERT(lizard_test_is_int(r, 2));
  r = lizard_test_eval(&e, "(hash-ref h 'age)");
  ...
  /* String keys. */
  r = lizard_test_eval(&e, "(define h2 (make-hash-table))"
                           "(hash-set! h2 \"alpha\" 1)"
                           "(hash-set! h2 \"beta\" 2)"
                           "(hash-set! h2 \"gamma\" 3)"
                           "(hash-ref h2 \"beta\")");
  ...
  r = lizard_test_eval(&e, "(define h3 (make-hash-table))"
                           "(hash-set! h3 1 'one)"
                           "(hash-set! h3 2 'two)"
  ...
  r = lizard_test_eval(&e, "(define big (make-hash-table))"
                           "(define (loop i n)"
                           "  (if (= i n) 'done"
  ...
  r = lizard_test_eval(&e, "(define (length xs)"
                           "  (if (null? xs) 0 (+ 1 (length (cdr xs)))))"
                           "(length (hash-keys big))");
  TEST_ASSERT(lizard_test_is_int(r, 200));

  /* hash-remove! actually removes. */
  r = lizard_test_eval(&e, "(hash-remove! big 50)"
                           "(hash-has-key? big 50)");
  TEST_ASSERT(lizard_test_is_false(r));
